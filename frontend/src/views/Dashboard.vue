...
onMounted(async () => {
  const products = await axiosInstance.get(`/products/`, config)
  productCount.value = products.data.length

  const customers = await axiosInstance.get(`/customers/`, config)
  customerCount.value = customers.data.length

  const invoices = await axiosInstance.get(`/invoices/`, config)
