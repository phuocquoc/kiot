...
const submitCustomer = async () => {
  try {
    if (editing.value) {
      await axios.put(`http://localhost:8000/api/customers/${editCustomerId.value}/`, customerForm.value, config)
      ElMessage.success('Cập nhật khách hàng thành công!')
    } else {
      await axios.post('http://localhost:8000/api/customers/', customerForm.value, config)
